...
fileName = [fileName_benchmark_500,fileName_benchmark_600,fileName_benchmark_700,fileName_benchmark_800,fileName_benchmark_1000]#,fileName_benchmark_1000_slep_110]
...
#Begin looping over DAOD's in list "fileName"
#read root file and access containers to show sim information
...
#    print "Event Number: ", event_counter
    ...
    for pp in p:
      if pp.absPdgId() in [5,511,521,531,10511,10521,513,523,10513,10523,20513,20523,515,525,10531,533,10533,20533,535,541,10541,543,10543,20543,545]:
        B.append(pp)
      break
    for pp in p:
    ...
    lepvector = leps_Hplus[0].p4() + leps_Hplus[1].p4() + leps_Hplus[2].p4()
...
#      leps_DR.append(leps[2])
    p4 = leps_neu[0].p4() + leps_neu[1].p4()
    ossfM_3lep.Fill(p4.M()/1000)

#    leps = sorted(leps, reverse = True, key=(lambda l: l.p4().Pt()))
#    if len(leps) > 3:
#      lepvector_b4 = leps[0].p4() + leps[1].p4() + leps[2].p4()
#      MET_b4.Fill((lepvector_b4 + metvector).Mt()/1000)
#      if leps[0].p4().DeltaR(leps[1].p4()) < leps[0].p4().DeltaR(leps[2].p4()):
#        if leps[0].pdgId() + leps[1].pdgId() != 0: continue
#        leps_b4_DR.append(leps[0])
#        leps_b4_DR.append(leps[1])
#      else:
#        if leps[0].pdgId() + leps[2].pdgId() != 0: continue
#        leps_b4_DR.append(leps[0])
#        leps_b4_DR.append(leps[2])
#      if len(leps_b4_DR) != 2: continue
#      p4_b4 = leps_b4_DR[0].p4() + leps_b4_DR[1].p4()
#      ossfM_b4removal_3lep.Fill(p4_b4.M()/1000)
    ...
    leps_new = sorted(leps_new, reverse = True, key=(lambda l: l.p4().Pt())) 
...
#    if len(set([abs(leps[0].pdgId()), abs(leps[1].pdgId()), abs(leps[2].pdgId())])) != 2: continue #2 ossf, one of different flavor
    lepvector_new = leps_new[0].p4() + leps_new[1].p4() + leps_new[2].p4()
    MET_new.Fill((lepvector_new + metvector).Mt()/1000) #met and pt_vis
    if leps_new[0].pdgId() + leps_new[1].pdgId() == 0:
      leps_new_DR.append(leps_new[0])
      leps_new_DR.append(leps_new[1])
    if leps_new[0].pdgId() + leps_new[2].pdgId() == 0:
      leps_new_DR.append(leps_new[0])
      leps_new_DR.append(leps_new[2])
    if len(leps_new_DR) != 2: continue
    p4_new = leps_new_DR[0].p4() + leps_new_DR[1].p4()
    ossfM_new_3lep.Fill(p4_new.M()/1000)
  ...
  legend.AddEntry(ossfM_3lep, "Leptons from H+")
  legend.AddEntry(ossfM_new_3lep, "From Leps List (with selection)")
  legend.AddEntry(ossfM_b4removal_3lep, "From Leps List (before selection)")
  ossfM_3lep.Draw("HIST")
  ossfM_b4removal_3lep.Draw("HIST SAME")
  ossfM_new_3lep.Draw("HIST SAME")
  ossfM_b4removal_3lep.SetLineColor(2)
  ossfM_new_3lep.SetLineColor(6)
  legend.Draw()
  canvas.Print("./plots/mllmax_"+str(file)+".png")
  MET.Draw("HIST")
  MET_b4.Draw("HIST SAME")
  MET_new.Draw("HIST SAME")
  MET_b4.SetLineColor(2)
  MET_new.SetLineColor(6)
  legend.Draw()
