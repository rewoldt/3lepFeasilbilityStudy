...
#  print "Number of input events:", t.GetEntries()
  ...
  DR_leading = ROOT.TH1F("DR_leading","DR Between Leading Lep and Any Other Lepton (w/ ossf)",71,0,5)
  DR_Hplus = ROOT.TH1F("DR_Hplus","DR Between all Lepton Combinations",71,0,5)
  ...
  DR_leading_selection = ROOT.TH1F("DR_leading_selection", "DR Between Leading Lep and Any Other Lepton (w/ossf)",71,0,5)
  DR_selection = ROOT.TH1F("DR_selection", "DR Between all Lepton Combinations (w/ ossf)", 71,0,5)
...
#DR between all lep combinations in event
    if leps_new[0].pdgId() + leps_new[1].pdgId() == 0:
      DR_selection.Fill(leps_new[0].p4().DeltaR(leps_new[1].p4()))
      DR_leading_selection.Fill(leps_new[0].p4().DeltaR(leps_new[1].p4()))
    if leps_new[0].pdgId() + leps_new[2].pdgId() == 0:
      DR_selection.Fill(leps_new[0].p4().DeltaR(leps_new[2].p4()))
      DR_leading_selection.Fill(leps_new[0].p4().DeltaR(leps_new[2].p4()))
    if leps_new[1].pdgId() + leps_new[2].pdgId() == 0:
      DR_selection.Fill(leps_new[1].p4().DeltaR(leps_new[2].p4()))
  ...
  legend.AddEntry(DR, "Before Selection")
  legend.AddEntry(DR_selection, "After Selection")
  legend.AddEntry(DR_Hplus, "From H+")
  DR_selection.Draw("HIST")
  ...
  DR_leading_selection.Draw("HIST")
  DR_leading_Hplus.Draw("HIST SAME")
